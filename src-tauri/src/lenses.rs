...
// Use case
// Fold InboxLens out to single inbox
// returns inbox array ---->   folding function
// InboxLens::fold<Inbox>(&test_proj, &|inbox_array| {
//  inbox_array.unwrap().iter().find(|ibox| ibox.id == upper_closure_inbox.id)
// }))
...
pub fn update_card(state: &State, project: String, inbox: String, card: Card) -> State {
    let found_project = ProjectLens::fold(state, &|projects| {
        Some(find_in(projects.unwrap(), project.to_owned()))
    })
    .unwrap();

    let found_inbox = InboxLens::fold(&found_project, &|ibox| {
        Some(find_in(ibox.unwrap(), inbox.to_owned()))
    })
    .unwrap();

    let found_card = CardLens::fold(&found_inbox, &|cards| {
...
pub fn create_card(state: &State, project: String, inbox: String, data: CardFragment) -> State {
    ...
    .unwrap();
    let found_inbox = InboxLens::fold(&found_project, &|boxes| {
        Some(find_in(boxes.unwrap(), inbox.to_owned()))
    })
    .unwrap();
